...
class TruststoreConstruct(Construct):
    def __init__(
        ...
        # Reference the existing S3 bucket (created by TruststoreBucketConstruct)
        ...
        # This is what ALB uses for mTLS validation
        self.trust_store = elbv2.CfnTrustStore(
            self,
            "TrustStore",
            name=f"PerseusResourceTrust-{environment_name}",
        ...
        CfnOutput(
            self,
            "TruststoreKey",
            value=truststore_key,
            description="Truststore S3 Key",
        )
