...
- The gas readings are **synthetic**, and the fixture says so in its own metadata. No public dataset carries half-hourly domestic gas volumes, so `resource/scripts/synthesise_gas_data.py` constructs them from Ofgem's medium gas Typical Domestic Consumption Value of 9,500 kWh a year, split 74.9% space heating, 22.0% hot water and 3.1% cooking after DESNZ's Energy Consumption in the UK Table U3, spread across the year by heating degree days on a 15.5C base against the real daily temperatures the premises' postcode district saw in the same year the electricity comes from, and across the day by when a boiler actually fires. Every constant in that script carries the source that justifies its value, and the fixture records the figures it was built from. The result swings about five and a half times harder between January and July than the electricity does, which is where most of a gas heated home's carbon variation lives. It should not be presented as data from a metered household
